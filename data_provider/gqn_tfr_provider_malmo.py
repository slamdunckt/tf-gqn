...
class DataReader(object):
  ...
  def __init__(self,
    ...
    with tf.device('/cpu'):
      ...
      iter =0
      read_ops = [self._make_read_op(reader, filename_queue, iter)
  ...
  def _make_read_op(self, reader, filename_queue, iter):
    ...
    feature_map = {
        'frames': tf.FixedLenFeature(
            shape=self._dataset_info.sequence_size[iter], dtype=tf.string),
        'x': tf.FixedLenFeature(
            shape=self._dataset_info.sequence_size[iter], dtype=tf.float32),
        'z': tf.FixedLenFeature(
            shape=self._dataset_info.sequence_size[iter], dtype=tf.float32),
        'yaw': tf.FixedLenFeature(
            shape=self._dataset_info.sequence_size[iter], dtype=tf.float32),
    }
  ...
  def _preprocess_cameras(self, example, indices):
    ...
    x =np.array(example['x'])
    y =np.zeros((x.shape[0],1))
    z =np.array(example['z'])
    yaw =np.array(example['yaw'])
    pitch =np.zeros((x.shape[0],1))
    pos = np.column_stack(x,y,z,yaw, pitch)
    raw_pose_params = pos.tolist()
...
class GQNTFRecordDataset(tf.data.Dataset):
  ...
  def __init__(self, dataset, context_size, root, mode='train',
    ...
    self._feature_map = {
        ...
        'x': tf.FixedLenFeature(
            shape=self._dataset_info.sequence_size, dtype=tf.float32),
        'z': tf.FixedLenFeature(
            shape=self._dataset_info.sequence_size, dtype=tf.float32),
        'yaw': tf.FixedLenFeature(
            shape=self._dataset_info.sequence_size, dtype=tf.float32),
